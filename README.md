...
```yaml
matrix:
  project: ["NUnitHyperTestDemo/MsTestHyperTestDemo.csproj"]
...
```yaml
post:
  - cat yaml/ms_hyperexecute_autosplit_sample.yaml
```
...
```bash
./hyperexecute --config yaml/ms_hyperexecute_autosplit_sample.yaml --force-clean-artifacts --download-artifacts
```
